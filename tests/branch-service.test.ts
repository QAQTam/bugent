...
describe("BranchService", () => {
  test("fork 从目标消息 inclusive 创建并激活新分支", () => {
    ...
    const result = service.fork(sessionId, 2);

    expect(result.sourceBranchId).toBe(branchId);
    expect(store.getActiveBranchId(sessionId)).toBe(result.branchId);
    expect(store.loadBranchPath(sessionId, result.branchId).map((m) => m.msgid)).toEqual([0, 1, 2]);
    expect(store.loadBranchPath(sessionId, branchId).map((m) => m.msgid)).toEqual([0, 1, 2, 3, 4]);
  });
  ...
  test("undo at 保留目标，undo before 排除目标，原分支都保留", () => {
    ...
    const preview = service.previewUndo(sessionId, 3, "at");
    expect(preview.retained.map((m) => m.msgid)).toEqual([0, 1, 2, 3]);
    expect(preview.removed.map((m) => m.msgid)).toEqual([4]);

    const at = service.undoTo(sessionId, 3, "at");
    expect(at.branchId).not.toBe(branchId);
    expect(store.loadBranchPath(sessionId, at.branchId).map((m) => m.msgid)).toEqual([0, 1, 2, 3]);

    const before = service.undoTo(sessionId, 3, "before");
    expect(store.loadBranchPath(sessionId, before.branchId).map((m) => m.msgid)).toEqual([0, 1, 2]);
    expect(store.loadBranchPath(sessionId, branchId).map((m) => m.msgid)).toEqual([0, 1, 2, 3, 4]);
  });
  ...
  test("retry 从最近 user 的父节点长分支，并返回原始输入", () => {
    ...
    const plan = service.planRetry(sessionId, 4);
    expect(plan.userMsgid).toBe(3);
    expect(plan.input).toBe("u2");
    expect(plan.retained.map((m) => m.msgid)).toEqual([0, 1, 2]);
    expect(plan.removed.map((m) => m.msgid)).toEqual([3, 4]);

    const result = service.retryFrom(sessionId, 4);
    expect(store.loadBranchPath(sessionId, result.branchId).map((m) => m.msgid)).toEqual([0, 1, 2]);
  });
  ...
  test("拒绝不在当前分支上的消息", () => {
    ...
    const { sessionId } = seed(store);
    const service = new BranchService(store);
    const fork = service.fork(sessionId, 2);
    open(store, sessionId).appendAssistant("fork-a");

    expect(() => service.previewUndo(sessionId, 4)).toThrow("不在当前活动分支");

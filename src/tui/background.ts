 ...
 * 终端底色探测。
 *
 * 为什么需要：`Bun.markdown.ansi` 会根据终端深浅给行内代码挑不同配色
 * （深色终端 `38;5;215`、浅色终端 `38;5;124`），而它判断深浅的方式是读
 * `COLORFGBG` —— 那个变量很多终端根本不导出，而且 Bun 只在**进程启动时**
 * 读一次。
 *
 * 所以 bugent 自己判定一次，再显式把 `light` 传给 Bun。这样结果与"Bun 什么
 * 时候读的环境变量"无关，也让将来接浅色主题时有一个现成的判定入口。
 * （行内代码的**底色**已被 bugent 主动剥掉，所以 `light` 现在只影响字色，
 * 但探测本身仍然必要。）
 *
 * 判定顺序：

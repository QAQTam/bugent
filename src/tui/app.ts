 ...
 * 滚动条是否出现，来回抖。固定留白，布局就是确定的。
 *
 * 留 5 列（滚动条本体仍在最右列，它左边 4 列是空的）是产品选择：正文与滚动条
 * 之间拉开距离，读的时候不会觉得文字顶到边上。代价是每多留 1 列，80 列终端就少
 * 1 列正文宽度 —— 从 76 降到 68（-11%）。
 */
export const SCROLLBAR_GUTTER = 5;
